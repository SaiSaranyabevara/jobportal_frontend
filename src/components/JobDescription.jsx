...
const JobDescription = () => {
  ...
  const applyJobhandler = async () => {
    try {
      ...
      setIsApplied(false); // Reset applied state on error
    }
  }
  ...
  useEffect(() => {
    const fetchSingleJob = async () => {
      try {
        ...
        // Handle error appropriately, e.g., dispatch an error action or show a notification
        dispatch(setSingleJob(null)); // Reset job data on error
    ...
    <div className='max-w-7xl mx-auto my-10'>
      <div className='flex items-center justify-between'>
        ...
        <Button
          ...
          disabled={isApplied} className={`rounded-lg ${isApplied ? 'bg-gray-600 cursor-not-allowed' : 'bg-[#7289b7] hover:bg-[#5f32ad]'}`}>{isApplied ? 'Already Applied' : 'Apply Now'}</Button>
      </div>
